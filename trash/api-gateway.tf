...
# definining our rest endpoint method and authorization here
resource "aws_api_gateway_method" "java_lambda_method" {
  rest_api_id   = "${aws_api_gateway_rest_api.java_lambda_api.id}"
  resource_id   = "${aws_api_gateway_resource.java_lambda_api_gateway.id}"
  # api gate way for any http method (GET, POST, PUT, DELETE .....)
  http_method   = "ANY"
  # no need any authorization while invoking our REST api
  authorization = "NONE"
}


# Integrating our api gateway to our lambda function
# our api gateway will invoke our lambda function in POST method
...
# defining our api gateway stage name
